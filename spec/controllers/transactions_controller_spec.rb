...
RSpec.describe TransactionsController, type: :controller do
  ...
  describe 'GET #index' do
    let(:transactions) do
      create_list(:transaction, 2, account_id: account.id, category_id: category.id, user_id: user.id, type_id: type.id)
    end
  ...
  describe 'PUT update' do
    ...
    context 'valid attributes' do
      let(:transaction) do
        create(:transaction, account_id: account.id, category_id: category.id, user_id: user.id, type_id: type.id)
      end
      ...
      it 'changes transaction attributes' do
        patch :update,
              params: { transaction: { title: 'New', body: 'News', cost: 321 }, account_id: account.id, user_id: user.id, type_id: type.id,
                        id: transaction.id }
        transaction.reload
  ...
  describe 'POST create' do
    ...
    context 'with valid attributes' do
      it 'saves the new transaction' do
        expect do
          post :create, params: { transaction: attributes_for(:transaction, category_id: category.id, type_id: type.id),
    ...
    context 'with invalid attributes' do
      it 'does not save the transaction' do
        expect do
          post :create,
               params: { transaction: attributes_for(:invalid_transaction), account_id: account.id, user_id: user.id, type_id: type.id,
  ...
  describe 'DELETE destroy' do
    ...
    it 'assigns @teams' do
      expect do
        delete :destroy,
               params: { id: transaction.id, account_id: account.id, user_id: user.id }
      end.to change(Transaction, :count).by(-1)
    end
  end
end
